...
function UnivariateSelection(mX::Array,vY::Vector,vNames,H::Vector{Int64},iStart::Int64,iBest::Int64,l::GLM.ProbitLink,fLoss)
    F = length(fLoss)
...
function sforecast(dfData::DataFrame,vSymbol::Array{Symbol,1},iSymbol::Symbol,H::Vector,iStart::Int64,iBest::Int64,ncomb_load::Int64,l::GLM.ProbitLink,fLoss::Array{Function})
    ...
    for f = 1:F
        counter += 1
        best_comb,factor_in = load_score(string(fLoss[1]),ncomb_load,H,U.vNames)
        model_index = Array{Vector}(size(best_comb,1))
        variables = Array{Vector}(size(best_comb,1))
        mFore    = zeros(size(vY,1),size(H,1))
        count = 0
    ...
    r = results(U,best_comb_vMae,factor_in_vMae,best_comb_vRmse,factor_in_vRmse) # MODIFY HERE
    return l_plot,r
end
...
type results
    ...
    best_comb_vMae::Array
    factor_in_vMae::Array
